(function () {
    ...
    var overlay = document.getElementById('amodal-overlay');
    if (!overlay || typeof tcAmodal === 'undefined') {
        return;
    }

    var card = document.getElementById('amodal-card');
    var viewForm = document.getElementById('amodal-view-form');
    var viewSuccess = document.getElementById('amodal-view-success');
    var form = document.getElementById('amodal-form');
    var closeX = document.getElementById('amodal-close-x');
    var closeBtn = document.getElementById('amodal-close-btn');
    var origenInput = document.getElementById('amodal-origen');
    var radioHelp = document.getElementById('amodal-radio-help');
    var errorGeneral = document.getElementById('amodal-error-general');
    var submitBtn = form.querySelector('.amodal-submit');
    var submitLabel = document.getElementById('amodal-submit-label');
    var submitLabelGhost = document.getElementById('amodal-submit-label-ghost');
    var rutInput = document.getElementById('amodal-rut');
    ...
    function clearErrors() {
        form.querySelectorAll('.amodal-error').forEach(function (el) {
            el.textContent = '';
            el.classList.remove('is-visible');
        });
        form.querySelectorAll('.amodal-input-invalid').forEach(function (el) {
            el.classList.remove('amodal-input-invalid');
        });
    }

    function showFieldError(field, message) {
        var input = form.querySelector('[name="' + field + '"]');
        var errorEl = form.querySelector('.amodal-error[data-error-for="' + field + '"]');
        if (input) {
            input.classList.add('amodal-input-invalid');
        }
        if (errorEl) {
    ...
    function updateRadioHelp() {
        ...
        form.querySelectorAll('.amodal-radio-pill').forEach(function (pill) {
    ...
    function openModal(trigger) {
        lastTrigger = trigger;
        origenInput.value = trigger.getAttribute('data-amodal-origen') || '';
        overlay.classList.add('is-open');
        overlay.setAttribute('aria-hidden', 'false');
        document.body.classList.add('amodal-open');
        document.addEventListener('keydown', onKeydown);

        var firstField = document.getElementById('amodal-nombre');
        if (firstField) {
            firstField.focus();
        }
    }
    ...
    document.querySelectorAll('[data-amodal-open]').forEach(function (trigger) {
    ...
    rutInput.addEventListener('input', function () {
        rutInput.classList.remove('amodal-input-invalid');
        var errorEl = form.querySelector('.amodal-error[data-error-for="rut"]');
        if (errorEl) {
    ...
    form.addEventListener('submit', function (event) {
        ...
        formData.append('nonce', tcAmodal.nonce);

        submitBtn.disabled = true;
        setSubmitLabel('Enviando…');

        fetch(tcAmodal.ajaxUrl, {
            method: 'POST',
