<?php
/**
 * Override de yourtheme/woocommerce/myaccount/form-edit-account.php
 * Base: woocommerce/templates/myaccount/form-edit-account.php (v11.0.0)
 *
 ...
 * intactos), únicamente para envolver el texto del botón "Save changes"
 * en el markup del roll de texto (.btn-label-mask + .btn-label +
 * .btn-label-ghost) — la duplicación de texto no se puede lograr solo
 * con CSS. Único cambio real respecto al core: el <button> final.
 */
	...
	<p>
		...
		<button type="submit" class="woocommerce-Button button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="save_account_details" value="<?php esc_attr_e( 'Save changes', 'woocommerce' ); ?>">
			<span class="btn-label-mask">
				<span class="btn-label"><?php esc_html_e( 'Save changes', 'woocommerce' ); ?></span>
				<span class="btn-label-ghost" aria-hidden="true"><?php esc_html_e( 'Save changes', 'woocommerce' ); ?></span>
			</span>
		</button>
